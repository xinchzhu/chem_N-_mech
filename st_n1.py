...
mech = 'AramcoMech3.0-m1'		# mechanism file name (exclude .cti suffix)
...
for i, tempu in enumerate(T_r):
    ...
    tau = ignitionDelay(timeHistory.t, timeHistory.P)
    t1 = time.time()
    timeHistory.write_csv('.\\timhis.dat',cols=('t','T','P','X'))
    IDTs[i] = tau
    print('Computed Ideal Gas Ignition Delay: {:.3} ms for T={}K. Took {:3.2f}s to compute'.format(tau*1000, tempu, t1-t0))

out = open('idt_%s_Phi%.1f_P%i.out'%(spec,equiv,pressu),'w')
