...
for steps_vtst in range(0, irc_steps):
    geo_inp = pd.read_csv('./irc_files/geo_%d.dat' % steps_vtst, delim_whitespace=True, header=None, skiprows=1,
                          engine='python')
    # geo_inp = pd.read_csv ('./irc_files/geo_0.dat',delim_whitespace = True,header = None, skiprows = 1,
    # engine = 'python')
    # print(geo_inp)
...
for steps_vtst in range(0, irc_steps):
    geo_temp2 = pd.read_csv('./mess_files/geo_temp1_%d' % steps_vtst, delim_whitespace=True, header=None,
                            engine='python')
    ...
    for i in range(0, rows_geo):
        ...
        elif geo_temp2.iloc[i, 0] == 8:            
            geo_temp2.iloc[i, 0] = 'O'
        elif geo_temp2.iloc[i, 0] == 14:
            geo_temp2.iloc[i, 0] = 'Si'
...
for i in range (0,energy_frame.shape[0]):
    irc_ene_0.append(energy_frame.iloc[i,1])
#print (irc_ene_0)
###
#####筛选频率 并计算zpe
ZPVE_har_out = []  #
...
for i in range(0, len(ZPVE_har_out)):
    temp_irc_zpve = ZPVE_har_out[i] - ZPVE_har_out[0]
    delta_irc_zpve.append(temp_irc_zpve)
print(delta_irc_zpve)
...
for steps_vtst in range(0, irc_steps):
    ...
    for i in range(0, freq_temp.shape[0]):
        if freq_temp.iloc[i, 0] != 0.0:
            freq_clean = freq_clean.append(freq_temp.iloc[i])
    freq_clean.to_csv('./mess_files/debug/freq_debug_%d' % steps_vtst, float_format='%.2f', index=False,
                      header=False, )
###################
time.sleep(1)
...
for i in range(0,irc_steps):
    ...
    if freq_counter != freqs:
        ...
        for lines in freq_inp.readlines():
            mess_out.write(lines)
        freq_inp.close()
    mess_out.write(' ZeroEnergy[kcal/mol]    %.5f\n' %(irc_ene_0[i]+delta_irc_zpve[i]))
    print (irc_ene_0[i]+delta_irc_zpve[i])
